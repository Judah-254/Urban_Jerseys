...
// Fetch Serie A jerseys from the database
$epl_jerseys_query = "SELECT * FROM products WHERE league = 'Serie A'";
...
<body>   
    <div class="container mt-5">
        <h3 class="text-center">Serie A Jerseys</h3>
        <div class="row">
            <?php while ($jersey = mysqli_fetch_assoc($epl_jerseys_result)): ?>
                <div class="col-md-4">
                    <div class="card mb-4">
                        <img src="images/<?php echo $jersey['image_url']; ?>" class="card-img-top" alt="<?php echo $jersey['name']; ?>">
                        <div class="card-body">
                        <h5 class="card-title"><?php echo $jersey['name']; ?></h5>
                            <p class="card-text"><?php echo htmlspecialchars($jersey['description']); ?></p> 
                            <p class="card-text"><strong>Price: KSH <?php echo number_format($jersey['price'], 2); ?></strong></p>
                            <a href="product_page.php?id=<?php echo $jersey['id']; ?>" class="btn btn-primary">View Product</a>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
